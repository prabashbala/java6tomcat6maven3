...
RUN echo 'deb http://ppa.launchpad.net/webupd8team/java/ubuntu trusty main' >> /etc/apt/sources.list && \
    ...
    rm -rf /var/cache/oracle-jdk${JAVA_VER}-installe && \
	rm -rf /var/lib/apt/lists/*

RUN mkdir "$TOMCAT_HOME" "$M2_HOME" && \
    ...
    wget -qO- "http://archive.apache.org/dist/tomcat/tomcat-8/v8.0.24/bin/apache-tomcat-8.0.24.tar.gz" | tar -zx --strip-components=1 -C "$TOMCAT_HOME" && \
    rm -rf "$TOMCAT_HOME"/webapps/* && \
    echo "export MAVEN_OPTS=\$JAVA_OPTS" >> /usr/.bashrc
